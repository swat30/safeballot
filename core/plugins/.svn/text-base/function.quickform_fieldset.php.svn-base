...
function smarty_function_quickform_fieldset($params, &$smarty) {
	...
	trigger_error("This is depreciated", E_USER_NOTICE);
	//var_dump ( $form );
	/* $form is required */
	if ((! isset ( $form ))) {
	...
	if (count ( $form ['errors'] )) {
		$html .= '<div><ul class="messagesList">';
	...
	$html .= "<fieldset" . (isset ( $id ) ? ' id="' . $id . '"' : null) . (isset ( $class ) ? ' id="' . $class . '"' : null) . ">";
	...
	for($i = 0; $i < count ( $fields ); $i ++) {
		...
		if (is_array ( $el ) && (! in_array ( 'html', array_keys ( $el ) ))) {
			...
			$el ['html'] = "\n\t\t<fieldset" . ' class="radiogroup" id="' . $el ['name'] . '">' . "\n\t\t\t<legend>" . $el ['label'] . "</legend>";
			...
			if (! stristr ( $el ['html'], ' id="' )) {
				$el ['html'] = str_replace ( ' name="' . $el ['name'] . '"', ' name="' . $el ['name'] . '" id="' . $el ['name'] . '"', $el ['html'] );
			}
			
			if (is_array ( $el ['label'] )) {
				$el ['label'] = $el ['label'] [0];
			}
			$el ['html'] = '<label for="' . $el ['name'] . '">' . $el ['label'] . '</label>' . $el ['html'] . '';
		}
		//ob_flush();
		//die();
		...
		if ('checkbox' == $el ['type'])
			$divclass [] = 'checkbox';
		if (! empty ( $divclass ))
			$class_text = ' class="' . implode ( ' ', $divclass ) . '"';
			
		//assemble the html string for this element
		$html .= "\n\t<li" . $class_text . '>' . $el ['html'] . "\n\t";
		if ($el ['error']) {
			$html .= '<img src="/images/admin/exclamation.png" />';
		}
		$html .="</li>";
